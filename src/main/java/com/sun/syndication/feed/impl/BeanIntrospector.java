...
import com.sun.syndication.feed.impl.PropertyDescriptor;
...
public class BeanIntrospector {
    ...
    public static synchronized PropertyDescriptor[] getPropertyDescriptors(Class klass) throws RuntimeException {
    ...
    private static PropertyDescriptor[] getPDs(Class klass) throws RuntimeException {
    ...
    private static Map getPDs(Method[] methods, boolean setters) throws RuntimeException {
        Map pds = new HashMap();
                ...
                if (setters) {
                    if (methods[i].getName().startsWith(SETTER) &&
                       methods[i].getReturnType() == void.class && methods[i].getParameterTypes().length == 1) {
                        pName = decapitalize(methods[i].getName().substring(3));
                    ...
                    if (methods[i].getName().startsWith(GETTER) &&
                       ...
                       methods[i].getReturnType() == boolean.class && methods[i].getParameterTypes().length == 0) {
                        pName = decapitalize(methods[i].getName().substring(2));
                        pDescriptor = new PropertyDescriptor(pName, methods[i], null);
                    }
                }
            }
    ...
    private static List merge(Map getters, Map setters) throws RuntimeException {
