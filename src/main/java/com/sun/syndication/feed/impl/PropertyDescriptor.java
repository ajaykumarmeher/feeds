...
public class PropertyDescriptor extends FeatureDescriptor {
    ...
    public PropertyDescriptor(String propertyName, Class<?> beanClass, String getterName,
                              String setterName) throws RuntimeException {
        super();
        if (beanClass == null) {
            throw new RuntimeException("beans.03"); //$NON-NLS-1$
        }
        if (propertyName == null || propertyName.length() == 0) {
            throw new RuntimeException("beans.04"); //$NON-NLS-1$
        }
        ...
        if (setterName != null) {
            if (hasMethod(beanClass, setterName)) {
                ...
                throw new RuntimeException("beans.20"); //$NON-NLS-1$
            }
        }
        if (getterName != null) {
            if (hasMethod(beanClass, getterName)) {
                ...
                throw new RuntimeException("beans.1F"); //$NON-NLS-1$
            }
        }
    }
    ...
    public PropertyDescriptor(String propertyName, Class<?> beanClass)
       throws RuntimeException {
        ...
        if (getter == null && setter == null) {
            throw new RuntimeException("beans.01" + propertyName); //$NON-NLS-1$
        }
    }

    public void setWriteMethod(Method setter) throws RuntimeException {
        if (setter != null) {
            int modifiers = setter.getModifiers();
            if (!Modifier.isPublic(modifiers)) {
                throw new RuntimeException("beans.05"); //$NON-NLS-1$
            }
            Class<?>[] parameterTypes = setter.getParameterTypes();
            if (parameterTypes.length != 1) {
                throw new RuntimeException("beans.06"); //$NON-NLS-1$
            }
            Class<?> parameterType = parameterTypes[0];
            Class<?> propertyType = getPropertyType();
            if (propertyType != null && !propertyType.equals(parameterType)) {
                throw new RuntimeException("beans.07"); //$NON-NLS-1$
            }
        }
        this.setter = setter;
    }

    public void setReadMethod(Method getter) throws RuntimeException {
        if (getter != null) {
            int modifiers = getter.getModifiers();
            if (!Modifier.isPublic(modifiers)) {
                throw new RuntimeException("beans.0A"); //$NON-NLS-1$
            }
            Class<?>[] parameterTypes = getter.getParameterTypes();
            if (parameterTypes.length != 0) {
                throw new RuntimeException("beans.08"); //$NON-NLS-1$
            }
            Class<?> returnType = getter.getReturnType();
            if (returnType.equals(Void.TYPE)) {
                throw new RuntimeException("beans.33"); //$NON-NLS-1$
            }
            Class<?> propertyType = getPropertyType();
            if ((propertyType != null) && !returnType.equals(propertyType)) {
                throw new RuntimeException("beans.09"); //$NON-NLS-1$
            }
        }
        this.getter = getter;
    }
    ...
    void setWriteMethod(Class<?> beanClass, String setterName) throws RuntimeException {
        boolean result = false;
    ...
    public PropertyEditor createPropertyEditor(Object bean) {
        ...
        try {
            ...
            RuntimeException re = new RuntimeException("beans.47", e); //$NON-NLS-1$
            throw re;
        }
        return editor;
    }
